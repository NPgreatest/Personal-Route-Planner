    ...
    <el-col span="8" v-if="showCard4">
      <el-card style="height: 600px;width: 1000px;margin-left: 30px;border-radius: 8px;">
        ...
        <div style="margin-top: 20px;">
          ...
          <el-button type="primary" @click="$router.push('/detail/' + app._id)">去调试-></el-button>
        </div>
      </el-card>
    </el-col>
  </el-row>
...
export default {
  ...
  methods: {
    async submitForm() {
      ...
      try {
        this.$store.dispatch('editApp/editApp', this.form).then((res) => {
  ...
  async mounted() {
    ...
    await this.$store.dispatch('getApp/getApp', { _id: id }).then((res) => {

    }).catch(() => {
      console.log('[HomeView]: 获取hello失败！')
    });
    this.form.name = this.app.name
    this.form.details = this.app.details
    this.form.knowledgebase = this.app.knowledgebase
